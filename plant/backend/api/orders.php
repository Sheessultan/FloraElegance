    ...
    case 'POST':
        ...
        if (isset($_GET['action']) && $_GET['action'] === 'verify') {
            ...
            if ($razorpaySignature === 'MOCK_PAYMENT_SIGNATURE') {
                try {
                    $mockCheck = $conn->prepare("SELECT id FROM orders WHERE id = ? AND user_id = ? AND status = 'pending' AND razorpay_order_id LIKE 'order_mock_%'");
                    $mockCheck->execute([$orderId, $userId]);
                    $signatureValid = (bool) $mockCheck->fetch();
                } catch (PDOException $e) {
                    $signatureValid = false;
                }
            } else {
            ...
            if ($signatureValid) {
                try {
                    ...
                    // Only mark paid from pending, owned by this user — prevents double capture & orphan stock updates
                    ...
                    if ($updateStmt->rowCount() !== 1) {
                        ...
                        echo json_encode([
                            "success" => false,
                            "message" => "Order could not be confirmed. It may not exist, already be paid, or belong to another account."
                        ]);
                        exit();
                    }

                    // Decrement stock levels for each item ordered
                ...
                try {
                    $failStmt = $conn->prepare("UPDATE orders SET status = 'failed' WHERE id = ? AND user_id = ? AND status = 'pending'");
                    $failStmt->execute([$orderId, $userId]);
        ...
        try {
            ...
            if ($httpcode === 200 && isset($responseData['id'])) {
                ...
                // If API call fails (no network or invalid dummy keys), fall back to beautiful mock testing ID
