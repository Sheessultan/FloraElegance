...
switch ($method) {
    ...
    case 'POST':
        ...
        // Try to get user name from token; fallback to guest name in payload
        ...
        } catch (Exception $e) { /* guest reviewer */ }

        try {
            $stmt = $conn->prepare("INSERT INTO product_reviews (product_id, user_id, user_name, user_image, rating, comment, status) VALUES (?,?,?,?,?,?,'approved')");
            $stmt->execute([
